...
export function UserNav() {
  ...
  if (!session) {
    return (
      <div className="flex items-center space-x-2">
        <ThemeToggle />
        <Button variant="ghost" size="sm" asChild className={onPrimary}>
  ...
  return (
    <div className="relative flex items-center" ref={menuRef}>
      ...
      {isMenuOpen && (
        <div
          className={`absolute right-0 mt-1 ${mainSecondary} rounded-md shadow-lg py-1 w-48 z-10 top-full`}
        >
          <div
            className={`px-3 py-2 text-sm ${onSecondary} border-b ${borderSecondary}`}
          >
            Signed in as{" "}
            <span className={`font-medium ${onSecondary}`}>
              {session.user?.email}
            </span>
          </div>
          <Link
