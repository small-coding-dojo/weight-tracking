...
export function ColorTokens() {
  ...
  const handleColorLoadWithTracking = useCallback(
              ...
              <div key={`${category.name},${index}`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {category.colors.map((color, colorIndex) => (
                    ...
                    >
                      <div
                        className={`h-16 rounded-md mb-2 ${colorClasses[`${color.name}${category.name}`]} border-2`}
                      >
                        {color.lightClassName.includes("text")
                          ? "Lorem ipsum dolor sit amet..."
                          : ""}
                      </div>
                      <div className="space-y-1">
                        <h3 className={`font-medium ${onSecondary}`}>
                          {color.name}
                        </h3>
                        {color.variable && (
                          <code
                            className={`text-sm ${backgroundSecondary} ${onSecondary} px-1 py-0.5 rounded`}
                          >
                            {color.variable}
                          </code>
                        )}
                        {color.description && (
                          <p
                            className={`text-sm ${subtleSecondary} ${onSecondary}`}
                          >
                            {color.description}
                          </p>
                        )}
                        <div
                          className={`text-xs mt-2 ${backgroundSecondary} ${onSecondary}`}
                        >
                          <div>
