...
export async function POST(req: NextRequest) {
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const measurement1 =
        row.measurement1 || row["measurement 1"] || row["Measurement 1"];
      const measurement2 =
        row.measurement2 || row["measurement 2"] || row["Measurement 2"];
      const measurement3 =
        row.measurement3 || row["measurement 3"] || row["Measurement 3"];
      ...
      try {
        ...
        if (isNaN(baseDate?.getTime?.() || NaN)) {
          errors.push(
            `Row ${rowNum}: Invalid date format - Could not parse "${dateValue}"`,
          );
          continue;
        }
      } catch (error) {
        const errorMessage =
          error instanceof Error ? error.message : "Unknown error parsing date";
      ...
      if (
        ...
        entries.push({
          value:
            typeof measurement1 === "number"
              ? measurement1
              : parseFloat(String(measurement1)),
          date: utcDate1,
          userId,
      ...
      if (
        ...
        entries.push({
          value:
            typeof measurement2 === "number"
              ? measurement2
              : parseFloat(String(measurement2)),
          date: utcDate2,
          userId,
      ...
      if (
        ...
        entries.push({
          value:
            typeof measurement3 === "number"
              ? measurement3
              : parseFloat(String(measurement3)),
          date: utcDate3,
          userId,
